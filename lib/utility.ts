...
utility.signURL = async function (url: string): Promise<string> {
  const signatureService =
    this.options.signatureService || DEFAULT_SIGNATURE_SERVICE;
...
async function post(urlStr: string, body: string): Promise<SignatureResponse> {
  ...
  return new Promise((resolve, reject) => {
    const req = http.request(requestArgs, (res) =>
...
export async function getBody(urlStr: string): Promise<object> {
  ...
  return new Promise((resolve, reject) => {
    const req = https.get(requestArgs, (res) =>
      handleResponse(res, resolve, reject)
    );
...
function handleResponse(
  ...
  res.on("end", () =>
    convertResponse(chunks, res.headers["content-encoding"], resolve, reject)
  );
}
...
async function convertResponse(
  ...
  if (string.length == 0) {
    reject(
      "TikTok sent back an empty response. This is not supposed to happen. Report it if you can."
    );
    return;
  }
...
export async function* getVideoGenerator(
  subset: BatchFunction,
  count: number,
  startCur: string,
  type: GeneratorType
): AsyncGenerator<VideoInfo[]> {
